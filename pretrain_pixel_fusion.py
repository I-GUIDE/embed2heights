...
DEFAULT_DATA = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "data"))
...
def parse_args():
    ...
    p.add_argument("--output-dir", default=os.path.join(SCRIPT_DIR, "runs"))
    ...
    p.add_argument("--norm-kind", default="gn", choices=["bn", "gn"],
                   help="LightUNet normalization layer. Default 'gn' is what "
                        "the supervised side should use too — BN running stats "
                        "do not transfer cleanly from label-free pretraining "
                        "(see SPLIT_TASK_REPORT BN/AMP failure).")
    p.add_argument("--mask-strategy", default="mixed",
                   choices=["complementary", "dual", "independent", "mixed"],
                   help="How alpha and tessera masks relate per batch. "
                        "'complementary' (mask one side, leave the other "
                        "visible) maximises cross-modal reconstruction "
                        "signal. 'dual' shares one mask. 'independent' is "
                        "the legacy behaviour. 'mixed' samples uniformly "
                        "among {complementary-alpha, complementary-tessera, "
                        "dual} per batch — recommended.")
...
def make_loaders(args, device):
    ...
    val_loader = DataLoader(val_ds, shuffle=False, **loader_kwargs) if val_pairs else None

% Embed2Heights --- current best framework, detailed walkthrough
...
\title{Embed2Heights: The Current Best Framework}
\subtitle{Dual-Trunk Cross-Source Fusion over Frozen GFM Embeddings}
\date{\today}
...
\begin{frame}{Dual LightUNet + learned gate \hfill\small\textcolor{cAlpha}{dense path}}
  \begin{center}
  \begin{tikzpicture}[node distance=4mm and 7mm]
    ...
    \node[sbox,fill=cAlpha!22,draw=cAlpha,minimum height=1.25cm,
          right=10mm of $(au)!0.5!(tu)$] (g) {gate $\sigma$};
  ...
  \end{center}
  \begin{itemize}
    \item \fit both are dense $256^2$ $\to$ a UNet yields sharp, IoU-friendly masks.
...
\begin{frame}{Backbone internals}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \textbf{LightUNet} (\code{base\_ch=48})
      \begin{itemize}\small
        \item 4 levels: $48\to96\to192\to384$.
        \item 3$\times$ MaxPool down ($256\to32$), bilinear up + skips.
...
\begin{frame}{Split-trunk multi-task head \hfill\small\textcolor{cHead}{head}}
  ...
  \begin{tikzpicture}[node distance=4mm and 9mm]
    ...
    \node[font=\tiny,cToken] at (sp) {\,$\times$\,};
    \node[font=\tiny,cToken,below=0pt of sp] {grad-cut};
  \end{tikzpicture}
  \end{center}
  \begin{itemize}
...
\begin{frame}{Head internals \& the gradient cut}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        \item forward identity:\\ $s\,t+(1{-}s)\,t.\text{detach}()$.
        \item $s\,{=}\,1$ coupled, $s\,{=}\,0$ hard cut.
        \item this is the knob the two-stage recipe flips.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Soft-bin height head \hfill\small\textcolor{cHead}{current best}}
...
\begin{frame}{Two-stage ``train then purify''}
  \begin{center}
  \begin{tikzpicture}[node distance=8mm]
    \node[shd,minimum width=3.2cm] (s1) {Stage 1: seg+height coupled\\\scriptsize 80 ep};
    \node[sop,right=6mm of s1] (c1) {ckpt\\$\to$ ch0--2};
    \node[shd,minimum width=3.2cm,right=10mm of c1] (s2)
         {Stage 2: seg-grad frozen,\\height re-tunes backbone\\\scriptsize 20 ep};
    \node[sop,right=6mm of s2] (c2) {ckpt\\$\to$ ch3};
    \draw[sar] (s1)--(c1);
    \draw[sar] (c1)--(s2) node[midway,above,font=\tiny,cGray]{init};
    \draw[sar] (s2)--(c2);
  \end{tikzpicture}
  \end{center}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{\textcolor{cHead}{Stage 1}} --- 80 ep, coupled\\
      \footnotesize batch 16, AdamW lr $2{\times}10^{-4}$, wd $10^{-4}$.\\
      train everything jointly.\\
      $\Rightarrow$ best ckpt supplies \textbf{ch 0--2}.
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{\textcolor{cHead}{Stage 2}} --- 20 ep, purify\\
      \footnotesize init from stage 1; \code{presence-grad-scale 0};
      lr $1.5{\times}10^{-4}$; select on height val loss.\\
      height trunk re-tunes the backbone.\\
      $\Rightarrow$ best ckpt supplies \textbf{ch 3}.
    \end{column}
  \end{columns}
  \vfill
  \fit the IoU/height conflict reaches the \emph{backbone}, not just the head
  $\Rightarrow$ lock segmentation, then re-purpose the backbone for height
  at $\sim\tfrac14$ a specialist's cost.
\end{frame}
...
\begin{frame}{The key alignment trick}
  \begin{block}{Presence target}
    \code{coverage > 0.10}\; (not argmax-of-fractions)
  \end{block}
  \begin{itemize}
    \item Reverse-engineered from the public board to match the \emph{official GT}.
    \item The old argmax rule over-counted buildings $\sim3\times$ --- our local
      bld IoU 0.543 was a \emph{mirage}; true $\approx0.475$.
    \item \fit identical inputs $\Rightarrow$ points come from matching the
      metric's exact masks, not more data.
  \end{itemize}
\end{frame}
...
\begin{frame}{Current best: \code{xf095\_2stage\_softbin\_covgt10}}
  Two-stage dual-trunk + shared soft-bin + presence target \code{cov>0.10}.
...
\begin{frame}{From one model to the final ensemble}
  ...
  \end{itemize}
  {\footnotesize U-Net++ was the only backbone that beat plain LightUNet under the
   corrected \code{cov>0.10} metric; heavier/attention encoders all lost.}
\end{frame}

\begin{frame}{Three purify stages off one stage-1}
  \begin{center}
  \begin{tikzpicture}[node distance=5mm and 8mm,font=\scriptsize]
    \node[shd,minimum width=2.6cm] (s1) {Stage 1 (coupled)\\\scriptsize 50--80 ep};
    \node[sop,above right=3mm and 11mm of s1] (hp) {height-purify\\\scriptsize seg-grad 0};
    \node[sop,below right=3mm and 11mm of s1] (sp) {seg-purify\\\scriptsize height-grad 0};
    \node[sbox,fill=cOut!14,draw=cOut,right=8mm of hp] (h3) {ch 3 (height)};
    \node[sbox,fill=cOut!14,draw=cOut,right=8mm of sp] (seg) {ch 0--2 (seg)};
    \draw[sar] (s1.east)|-(hp); \draw[sar] (s1.east)|-(sp);
    \draw[sar] (hp)--(h3); \draw[sar] (sp)--(seg);
  \end{tikzpicture}
  \end{center}
  \begin{itemize}\small
    \item \textbf{height-purify} (\code{presence-trunk-grad-scale 0}): freeze the
      seg side, height re-tunes the backbone $\Rightarrow$ best \textbf{ch 3}.
    \item \textbf{seg-purify} (\code{height-trunk-grad-scale 0}): the \emph{mirror}
      --- freeze height, seg owns the backbone $\Rightarrow$ sharper
      \textbf{ch 0--2}. \alert{+0.0044 public building IoU.}
    \item \fit IoU and RMSE fight over the \emph{shared} backbone; give each a
      dedicated 20-ep fine-tune off the same stage-1 checkpoint.
  \end{itemize}
\end{frame}
...
\begin{frame}{clDice --- a topology loss for merged buildings}
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}\small
        \item soft-skeleton via iterated min/max-pool; centerline Dice on the
          building presence map.
        \item rewards \emph{connectivity}, penalises merges --- orthogonal to the
          area (Tversky) losses.
        \item added as a 20-ep purify on top of seg-purify.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.3em}
  {\footnotesize Ensemble building IoU $0.5039\!\to\!0.5072$ OOF.
   \textcolor{cToken}{Refuted:} clDice baked into \emph{stage-1} --- it pulled the
   backbone toward topology and \emph{degraded height} (RMSE$_{bH}$ +0.006).
   Late-purify clDice keeps height untouched.}
\end{frame}
...
\begin{frame}{Height correction: additive shift (kept) vs.\ de-compression (refuted)}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Additive shift} \textcolor{cHead}{(kept)}
      \begin{itemize}\small
        \item $h\!\to\!h+c$ on predicted-class pixels.
        \item building $+0.30$ (public bias $-0.31$), veg $+0.12$.
        \item corrects the \emph{mean} bias; region-invariant $\Rightarrow$
          private-safe. RMSE$_{bH}$ $1.807\!\to\!1.778$.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{De-compression} \textcolor{cToken}{(refuted)}
      \begin{itemize}\small
        \item $h\!\to\!1.05h+0.116$ (slope $>1$, un-squeeze the tall tail).
        \item motivated by the strong compression curve ($-4.5$\,m @ 15\,m).
        \item but test is only \emph{mildly} taller $\Rightarrow$ over-corrects
          mid-height $\Rightarrow$ RMSE$_{bH}$ $1.778\!\to\!1.782$.
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{0.3em}
  {\footnotesize Lesson: the recoverable height error is a \emph{mean offset}, not
   a slope --- the additive shift already captures it; the residual $+0.33$\,m is
   irreducible region-shift scatter.}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item clDice-from-stage-1; height de-compression
...
\begin{frame}{Summary --- each design answers ``what is the input?''}
  ...
  \begin{tabular}{ll}
    ...
    \midrule
    Dual LightUNet + gate & dense $256^2$, differing reliability\\
    ...
    cov$>$0.10 + per-class thr & match the metric's exact definition\\
    \bottomrule
  \end{tabular}
\end{frame}

\begin{frame}[standout]{Take-away}
  Same frozen embeddings for everyone $\Rightarrow$ every design is justified by
  \emph{what the embedding is} (dense vs.\ coarse, its scale, its reliability)
  and \emph{what the metric measures}.
\end{frame}

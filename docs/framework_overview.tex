...
\title{Fuse and Sharpen: Cross-Embedding Fusion with Boundary Recovery for Height \& Land Cover}
...
\begin{frame}{Two-stage: train, then purify (1 height + 2 seg)}
  ...
  \begin{tikzpicture}[font=\scriptsize,node distance=4mm and 10mm]
    \node[shd,minimum width=2.4cm] (s1) {Stage 1\\coupled, 50 ep};
    \node[sop,above right=4mm and 11mm of s1] (hp) {height-purify\\\scriptsize seg-grad 0, 20 ep};
    \node[sop,below right=4mm and 11mm of s1] (sp) {seg-purify\\\scriptsize height-grad 0, 20 ep\\$+$ 20 ep clDice};
    \node[sbox,fill=cOut!14,draw=cOut,right=10mm of hp] (h3) {ch 3};
    \node[sbox,fill=cOut!14,draw=cOut,right=10mm of sp] (seg) {ch 0--2};
    \draw[sar] (s1.east) |- (hp.west);
    \draw[sar] (s1.east) |- (sp.west);
  ...
  \begin{columns}[T]
    \begin{column}{0.4\textwidth}
      \textbf{\textcolor{cHead}{Stage 1}} --- coupled, 50 ep\\
      \footnotesize AdamW lr $2{\times}10^{-4}$; train everything jointly
      $\Rightarrow$ a \emph{shared init} for all purifies.
    \end{column}
    \begin{column}{0.6\textwidth}
      ...
      \end{itemize}
      $\Rightarrow$ the seg-purify \emph{and} clDice ckpts are both kept
      (2 decorrelated seg sources $\to$ 50-member seg).
    \end{column}
  \end{columns}
  \vfill
  \fit IoU and RMSE fight over the \emph{backbone}, not just the head $\Rightarrow$ a
  cheap 20-ep purify per task, seg run twice (plain \& clDice), all off one stage-1.
\end{frame}
...
\begin{frame}{Three purify stages off one stage-1}
  ...
  \begin{tikzpicture}[font=\scriptsize,node distance=4mm and 9mm]
    \node[shd,minimum width=2.4cm] (s1) {Stage 1 (coupled)\\\scriptsize 50 ep};
    \node[sop,above right=4mm and 10mm of s1] (hp) {height-purify\\\scriptsize seg-grad 0, 20 ep};
    \node[sop,below right=4mm and 10mm of s1] (sp) {seg-purify\\\scriptsize height-grad 0, 20 ep\\$+$ 20 ep clDice};
    \node[sbox,fill=cOut!14,draw=cOut,right=10mm of hp] (h3) {ch 3 (height)};
    \node[sbox,fill=cOut!14,draw=cOut,right=10mm of sp] (seg) {ch 0--2 (seg)};
    \draw[sar] (s1.east) |- (hp.west);
    \draw[sar] (s1.east) |- (sp.west);
    \draw[sar] (hp.east) -- (h3.west);
    \draw[sar] (sp.east) -- (seg.west);
  ...
  \begin{itemize}\small
    ...
    \item \textbf{$+$clDice}: a 3rd fine-tune continuing the \emph{seg-purify} ckpt with
      the clDice topology loss (next slide) $\Rightarrow$ a 2nd, decorrelated seg source.
  \end{itemize}
\end{frame}

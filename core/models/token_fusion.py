...
from .backbones import LightUNet, LightUNetPP, LightUNet3Plus, HRNet
...
def _build_pixel_backbone(kind, in_channels, n_classes, base_ch, norm_kind,
    ...
    if k in ("unet3plus", "unet3+", "unet+++", "lightunet3plus"):
        ...
        return LightUNet3Plus(in_channels, n_classes, base_ch=base_ch, norm_kind=norm_kind)
    if k in ("hrnet", "hrnetv2"):
        # HRNetV2: parallel multi-resolution branches with repeated cross-scale
        # fusion (no encoder/decoder bottleneck). Strongest boundary-preserving
        # member of the dense-skip family that won under cov>0.10 (unetpp/unet3+).
        # forward_features returns base_ch @ full-res -> drop-in for LightUNet.
        return HRNet(in_channels, n_classes, base_ch=base_ch, norm_kind=norm_kind)
    ...
    raise ValueError(
        ...
        "'unet3plus', 'wavelet', 'unetpp_wavelet', or 'hrnet'."
    )
...
class GatedPixelFusionHybridLightUNet(nn.Module):
    ...
    def __init__(self, pixel_channels, token_channels, n_classes=4,
        ...
        self.unetpp_deep_supervision = bool(unetpp_deep_supervision)
        ...
        self.alpha_unet = _build_pixel_backbone(
            ...
            deep_supervision=self.unetpp_deep_supervision,   # alpha backbone only
    ...
    def forward(self, x, return_aux=False):
        ...
        if self.n_head_replicas == 1:
            ...
            out = self.head(fused, return_aux=return_aux, presence_extra=presence_extra)
            if (return_aux and isinstance(out, dict)
                    and getattr(self.alpha_unet, "_ds_outputs", None) is not None):
                out["ds_outputs"] = self.alpha_unet._ds_outputs   # U-Net++ deep-sup aux maps
            return out

"""PyTorch dataset for competition raster embeddings.

The pipeline always fuses two pixel sources (AlphaEarth + Tessera) with N token
sources (TerraMind S1/S2, Thor S1/S2) against the 4-channel labels, so a single
dataset class is needed.
"""
...
def _pad_pixel_training_tensors(image, target, valid_mask, patch_size):
    ...
    valid_mask = _pad_to_min_shape(
        valid_mask, (patch_size, patch_size), mode="constant", constant_values=0,
    )
...
class PixelMultiTokenEmbeddingDataset(Dataset):
    """Two pixel sources (AlphaEarth+Tessera) + N token sources against the labels.

    file_pairs items are:
      - (primary_emb, secondary_emb, *token_embs, label)          training / val
      - (primary_emb, secondary_emb, *token_embs)                 label-free inference

    Returns ((pixel_image, token_image), target, valid_mask), where pixel_image is
    AlphaEarth+Tessera concatenated at patch_size and token_image is the token
    sources channel-concatenated at patch_size / scale_factor.
    """

    def __init__(self, file_pairs, patch_size=128, scale_factor=16, is_train=True,
                 missing_mask_dir=None):
    ...
    def __getitem__(self, idx):
        ...
        for token_path, token_array in zip(token_paths[1:], token_arrays[1:]):
            _assert_same_spatial(token_arrays[0], token_array, token_paths[0], token_path, "Token")
        ...
        pixel, target, valid_mask = _pad_pixel_training_tensors(
            pixel, target, valid_mask, self.patch_size,
        )

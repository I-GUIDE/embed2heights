...
def parse_args():
    ...
    p.add_argument("--unetpp-deep-supervision", action=argparse.BooleanOptionalAction,
                   default=False,
                   help="U-Net++ deep supervision: lightweight (presence+height) "
                        "aux heads on the shallow nested nodes X^0_1/X^0_2 of the "
                        "ALPHA backbone, supervised in the loss. Training-only.")
    p.add_argument("--unetpp-ds-weight", type=float, default=0.0,
                   help="Overall lambda for the U-Net++ deep-supervision aux loss. "
                        "0 disables (default).")
    p.add_argument("--unetpp-ds-height-weight", type=float, default=0.0,
                   help="Weight of the height L1 term within each deep-sup aux "
                        "head (0 => presence-only deep supervision).")
    p.add_argument("--cl-dice-weight", type=float, default=0.0,
                   help="clDice (centerline-Dice) topology loss weight on the "
                        "building presence map. Targets merged-building FP. "
                        "0 disables (default).")
    p.add_argument("--cl-dice-iters", type=int, default=5,
                   help="Soft-skeleton iterations for clDice (default 5).")
    p.add_argument("--unetpp-bottleneck-attn", action=argparse.BooleanOptionalAction,
                   default=False,
                   help="TransUNet: global self-attention transformer at the "
                        "U-Net++ encoder bottleneck (X^3_0, 32x32). Zero-init "
                        "residual (baseline-identical at start). Applied to both "
                        "alpha + tessera pixel backbones.")

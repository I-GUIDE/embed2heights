...
> **Current best local validation ensemble:** `ensemble_Q_Qpretrain_N_Qterramind_s2_mean` at `0.4813` raw / **`0.4846` per-class TS** with thresholds `(0.620, 0.530, 0.690)` — 4-way mean of Q, Q-pretrain, N, and Q+TerraMind-S2. Public score: **`0.4282`**. See [ENSEMBLE_Q_QPRETRAIN_N_QTERRAMIND_REPORT.md](ENSEMBLE_Q_QPRETRAIN_N_QTERRAMIND_REPORT.md).
> **Current raw single-model champion:** `alphaearth_tessera_iou_fusion_N_base48` at val `0.4644` raw / `0.4673` per-class threshold-swept — LightUNet widened to `base_ch=48` on top of J's loss recipe. See [N_O_BASECH_WIDENING_REPORT.md](N_O_BASECH_WIDENING_REPORT.md).
> **Current threshold-swept single-model best:** `alphaearth_tessera_iou_fusion_Q_base48_fused_height_gate_pretrain_m55` at val `0.4710` raw / **`0.4745` per-class TS** — Q initialized from train+test self-supervised AlphaEarth+Tessera pretraining. Positive but moderate lift over Q (`+0.0064` raw / `+0.0033` TS); treat as a strong ensemble candidate, not a clean replacement for N as raw champion. See [PRETRAIN_AE_TESSERA_REPORT.md](PRETRAIN_AE_TESSERA_REPORT.md).
...
| **Height Routing** | N raw champion uses AlphaEarth-only gate; Q variant uses fused AlphaEarth+Tessera gate | Q validates fused routing for height (`RMSE_vH 3.3963→3.3286`) but shifts IoU calibration. Keep N for raw baseline, Q for TS / ensemble. See [Q_R_HEIGHT_ROUTING_REPORT.md](Q_R_HEIGHT_ROUTING_REPORT.md) |
| **Fusion** | AlphaEarth primary + compressed Tessera residual presence branch (`hidden_ch=96, depth=2`) | Tessera corrects submitted presence / IoU logits; Q additionally lets that correction route height |
...
- [x] **Q fused height gate + AE/Tessera pretrain** — best single-model per-class TS so far (`0.4745`), mostly by lifting building IoU and height RMSE. The non-pretrained Q remains the cleaner architecture ablation (`0.4712` TS). See [PRETRAIN_AE_TESSERA_REPORT.md](PRETRAIN_AE_TESSERA_REPORT.md).
...
- [ ] TTA (flip + D4) on N via [predict_tta.py](../predict_tta.py) — near-free +0.005–0.01
...
- Current best public score is `0.4282` from the threshold-baked 4-way Q/Q-pretrain/N/Q-TerraMind-S2 ensemble. Local val still overestimates public by about `0.056` for this submission.
...
- Q is the current best single-model threshold-swept validation run, but it does not clear the promotion bar as a raw champion: raw gain is within noise and raw IoUs regress. Treat it as a high-value ensemble / threshold-baked candidate.
- Q + AE/Tessera self-supervised pretrain is now the best threshold-swept local-val single model (`0.4745`). The lift is real but moderate (`+0.0033` TS over Q), and water IoU moves slightly down; prioritize testing the same initialization on N before changing the primary champion policy.
- The best current local-val submission candidate is the threshold-baked 4-way ensemble `Q + Q_pretrain + N + Q_TerraMind-S2` (`0.4846` TS). Its gain comes from all five metrics improving after averaging, especially water IoU and height RMSE.
